...
class FoundationPose:
  ...
  def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    ...
    '''
    set_seed(0)
    #logging.info('Guess the first pose object')
...
class FoundationPoseMulti:
  ...
  def register_multi(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    '''Copmute pose from given pts to self.pcd
    @pts: (N,3) np array, downsampled scene points
    '''
    set_seed(0)
    ...
    normal_map = None
    print(f"{(depth >= 0.1) & (ob_mask>0)} | {depth >= 0.1}, {ob_mask>0}")
    valid = (depth>=0.1) & (ob_mask>0)
    if valid.sum()<4:
      #logging.info(f'valid too small, return')
      pose = np.eye(4)
    ...
    best_pose = poses[0]@self.get_tf_to_centered_mesh()
    best_pose = best_pose.to(torch.float)
    self.pose_last[ob_id] = poses[0]
    self.best_id[ob_id] = ids[0]
    self.poses[ob_id] = poses
    self.scores[ob_id] = scores

    return best_pose.data.cpu().numpy()

  def register_multi_return_tmp(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    '''Copmute pose from given pts to self.pcd
    @pts: (N,3) np array, downsampled scene points
    '''
    set_seed(0)
  ...
  def track_one_multi(self, rgb, depth, K, iteration, ob_id, extra={}):
    if ob_id not in self.pose_last:
      #logging.info(f"Please init pose for object {ob_id} by register first")
      raise RuntimeError
    #logging.info(f"Tracking object {ob_id}")
